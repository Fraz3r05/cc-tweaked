...
local function yPos(self)
    turnTo(nil, 1)
    refuel(nil)
    turtle.forward()
    y = y + 1
end
local function yNeg(self)
    turnTo(nil, 3)
    refuel(nil)
    turtle.forward()
    y = y - 1
...
local function moveTo(self, _x, _y, _z)
    if _z > z then
        do
            local n = 1
            while n <= _z - z do
...
function ____exports.placeAtFacing(self, itemName, _x, _y, _z, face)
    if face == -1 then
        moveTo(nil, _x, _y, _z + 1)
        place(nil, itemName, true)
    else
        if face == 0 then
            moveTo(nil, _x - 1, _y, _z)
        elseif face == 1 then
            moveTo(nil, _x, _y - 1, _z)
        elseif face == 2 then
            moveTo(nil, _x + 1, _y, _z)
        elseif face == 3 then
            moveTo(nil, _x, _y + 1, _z)
...
--[[ Generated with https://github.com/TypeScriptToLua/TypeScriptToLua ]]
local ____exports = {}
local ____grid = require("grid")
local grid = ____grid.grid
local ____building = require("building")
    ...
    while n <= #grid do
        do
            ...
            while y <= #grid[n + 1] do
                do
                    local x = 1
                    while x <= #grid[n + 1][y + 1] do
                        local item = grid[n + 1][y + 1][x + 1]
                        if item < 5 then
                            placeAtFacing(
                                nil,
                                names[item + 1],
...
print("needed:")
print("Dense Cable: " .. tostring(denseCount))
print("Cable: " .. tostring(cableCount))
print("Growth Accelerator: " .. tostring(growthCount))
print("Field: " .. tostring(fieldCount))
return ____exports
 end,
}
return require("main", ...)
