...
local z = 0
local orientation = 0
function ____exports.getPos(self)
    return {x = x, y = y, z = z}
end
local function refuel(self)
    local fuelLevel = turtle.getFuelLevel()
    if fuelLevel == "unlimited" or fuelLevel > 0 then
        return
    end
    local function tryRefuel(self)
        do
            local n = 1
            while n <= 16 do
    ...
    if not tryRefuel(nil) then
        print("Add more fuel to continue...")
        while not tryRefuel(nil) do
...
local function turnTo(self, face)
...
local function xPos(self)
    turnTo(nil, 0)
    refuel(nil)
    turtle.forward()
    x = x + 1
end
local function xNeg(self)
    turnTo(nil, 2)
    refuel(nil)
    turtle.forward()
    x = x - 1
end
local function zPos(self)
    turnTo(nil, 1)
    refuel(nil)
    turtle.forward()
    z = z + 1
end
local function zNeg(self)
    turnTo(nil, 3)
    refuel(nil)
    turtle.forward()
    z = z - 1
end
local function place(self, itemName, isDown)
    local function tryPlace(self)
        do
            local slot = 1
            while slot <= 16 do
    ...
    if not tryPlace(nil) then
        print(("Add " .. itemName) .. " blocks")
        while not tryPlace(nil) do
            os.pullEvent("turtle_inventory")
        end
        print("Resume...")
    end
end
function ____exports.moveTo(self, _x, _y, _z)
    if _y > y then
        while y ~= _y do
            up(nil)
        end
...
function ____exports.placeAtFacing(self, itemName, _x, _y, _z, face)
    if face == -1 then
        ____exports.moveTo(nil, _x, _y + 1, _z)
        place(nil, itemName, true)
    else
        if face == 0 then
            ____exports.moveTo(nil, _x - 1, _y, _z)
        elseif face == 1 then
            ____exports.moveTo(nil, _x, _y, _z - 1)
        elseif face == 2 then
            ____exports.moveTo(nil, _x + 1, _y, _z)
        elseif face == 3 then
            ____exports.moveTo(nil, _x, _y, _z + 1)
        ...
        turnTo(nil, face)
        place(nil, itemName, false)
    end
end
return ____exports
 end,
["src.tower.tower"] = function(...) 
--[[ Generated with https://github.com/TypeScriptToLua/TypeScriptToLua ]]
local ____exports = {}
local ____grid = require("src.tower.grid")
local grid = ____grid.grid
local ____building = require("src.lib.building")
local moveTo = ____building.moveTo
local placeAtFacing = ____building.placeAtFacing
function ____exports.main(self)
    local names = {"ae2:black_smart_dense_cable", "ae2:fluix_glass_cable", "ae2:growth_accelerator", "mysticalagradditions:insanium_farmland"}
    local floors = 4
    do
        local floor = 0
        while floor < floors + 1 do
            moveTo(nil, -2, 1, 2)
            do
                local n = 0
                while n < (floor == floors and 1 or #grid) do
                    do
                        local z = 0
                        while z < #grid[n + 1] do
                            do
                                local x = 0
                                while x < #grid[n + 1][z + 1] do
                                    local item = grid[n + 1][z + 1][x + 1]
                                    if item >= 0 then
                                        placeAtFacing(
                                            nil,
                                            names[item + 1],
...
--[[ Generated with https://github.com/TypeScriptToLua/TypeScriptToLua ]]
local ____exports = {}
local ____tower = require("src.tower.tower")
local main = ____tower.main
main(nil)
return ____exports
 end,
}
